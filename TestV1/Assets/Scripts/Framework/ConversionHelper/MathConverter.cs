...
namespace ConversionHelper
{
    ...
    public static class MathConverter
    {
        ...
        public static void Convert(Vector3 xnaVector, out BEPUutilities.Vector3 bepuVector)
        {
            bepuVector.X = (Fix64)xnaVector.x;
            bepuVector.Y = (Fix64)xnaVector.y;
        ...
        public static BEPUutilities.Vector3[] Convert(Vector3[] xnaVectors)
        {
            ...
            {
                Convert(xnaVectors[i], out bepuVectors[i]);
            }
            return bepuVectors;
        ...
        public static BEPUutilities.Ray Convert(Ray ray)
        {
            ...
            Convert(ray.origin, out toReturn.Position);
            Convert(ray.direction, out toReturn.Direction);
            return toReturn;
        }

        public static void Convert(ref Ray ray, out BEPUutilities.Ray bepuRay)
        {
            Convert(ray.origin, out bepuRay.Position);
            Convert(ray.direction, out bepuRay.Direction);
        }

        public static Ray Convert(BEPUutilities.Ray ray)
        {
            Convert(ref ray.Position, out Vector3 origin);
            Convert(ref ray.Direction, out Vector3 direction);
        ...
        public static void Convert(ref BEPUutilities.Ray ray, out Ray xnaRay)
        {
            Convert(ref ray.Position, out Vector3 origin);
            Convert(ref ray.Direction, out Vector3 direction);
            xnaRay = new Ray(origin, direction);
        }

        //BoundingBox
        public static Bounds Convert(BEPUutilities.BoundingBox boundingBox)
        {
            Convert(ref boundingBox.Min, out Vector3 umin);
            Convert(ref boundingBox.Max, out Vector3 umax);
            Bounds toReturn = new();
            toReturn.SetMinMax(umin, umax);
            return toReturn;
        }
        ...
        public static void Convert(ref BEPUutilities.BoundingBox boundingBox, out Bounds xnaBoundingBox)
        {
            Convert(ref boundingBox.Min, out Vector3 umin);
            Convert(ref boundingBox.Max, out Vector3 umax);
            xnaBoundingBox = new Bounds();
            xnaBoundingBox.SetMinMax(umin, umax);
        }
        ...
        public static BEPUutilities.BoundingSphere Convert(BoundingSphere boundingSphere)
        {
            ...
            Convert(boundingSphere.position, out toReturn.Center);
        ...
        {
            Convert(boundingSphere.position, out bepuBoundingSphere.Center);
